...
public class OSBHighscoresParser {
	...
	private final String[] SKILLS = { "attack_xp", "defence_xp", "strength_xp", "hitpoints_xp", "ranged_xp",
			"prayer_xp", "magic_xp", "cooking_xp", "woodcutting_xp", "fletching_xp", "fishing_xp", "firemaking_xp",
			"crafting_xp", "smithing_xp", "mining_xp", "herblore_xp", "agility_xp", "thieving_xp", "slayer_xp",
			"farming_xp", "runecrafting_xp", "hunter_xp", "construction_xp" };
	private final String[] PRESTIGES = { "attack_prestiges", "defence_prestiges", "strength_prestiges",
			"hitpoints_prestiges", "ranged_prestiges", "prayer_prestiges", "magic_prestiges", "cooking_prestiges",
			"woodcutting_prestiges", "fletching_prestiges", "fishing_prestiges", "firemaking_prestiges",
			"crafting_prestiges", "smithing_prestiges", "mining_prestiges", "herblore_prestiges", "agility_prestiges",
			"thieving_prestiges", "slayer_prestiges", "farming_prestiges", "runecrafting_prestiges", "hunter_prestiges",
			"construction_prestiges" };
	public static final int[] EXP_FOR_LEVEL = { 0, 83, 174, 276, 388, 512, 650, 801, 969, 1154, 1358, 1584, 1833, 2107,
	...
	public void parse(JsonObject reader) {
		...
		OSBLoader.stat_combat
				.setText(String.valueOf(calculateCombat(attack, defence, strength, hp, ranged, prayer, magic)));

		for (int i = 1; i < SKILLS.length; i++) {
			if (reader.has(SKILLS[i])) {
				Client.experience[i] = Integer.parseInt(reader.get(SKILLS[i - 1]).getAsString());
				OSBLoader.stat[i].setText(String
						.valueOf(getLevelForExperience(Integer.parseInt(reader.get(SKILLS[i - 1]).getAsString()))));
			}
		}
	...
	public void deserialize(String username) {
		try (BufferedReader reader = new BufferedReader(new InputStreamReader(
				new URL("http://runity.io/osbuddy/highscores.php?username=" + username).openStream(), "UTF8"))) {
	...
	private static double calculateCombat(int attack, int defence, int strength, int hp, int ranged, int prayer,
			int magic) {
