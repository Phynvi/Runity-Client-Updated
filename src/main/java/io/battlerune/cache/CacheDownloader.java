...
public class CacheDownloader {
	...
	public static void init(boolean force) {
		...
		if (latest > current || force) {
			try {
				 ...
				 */
				Unzip.unzip(new File(Utility.findcachedir() + CACHE_NAME));

				/**
				 * Write new version
				 */
				File version = new File(Utility.findcachedir() + "version.txt");
	...
	private static void download() throws Exception {
		...
		if (responseCode == HttpURLConnection.HTTP_OK) {
			...
			String saveFilePath = Utility.findcachedir() + File.separator + CACHE_NAME;
			...
			while ((bytesRead = inputStream.read(buffer)) != -1) {
				...
				Client.instance.drawLoadingText(percentage,
	...
	private static double getCurrentVersion() {
		double version = 1.5;
		try {
			File file = new File(Utility.findcachedir() + "version.txt");
