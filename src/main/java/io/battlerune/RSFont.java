...
public class RSFont extends Raster {
	...
	private void drawCharacter(byte charPixels[], int x, int y, int width, int height, int color) {
		...
		if (x + width >= bottomX) {
			int l2 = ((x + width) - bottomX);
			width -= l2;
			l1 += l2;
			k1 += l2;
		}
		if (!(width <= 0 || height <= 0)) {
			createPixels(pixels, charPixels, color, pixel, offset, width, height, k1, l1);
		}
	}
	...
	public void drawCharacter(int character, int x, int y, int width, int height, int shadowColor, boolean bool) {
		int i_40_ = x + y * Raster.width;
