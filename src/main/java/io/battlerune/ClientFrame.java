...
public class ClientFrame extends Client {
			...
			frame.addWindowListener(new WindowAdapter() {
				...
				public void windowClosing(WindowEvent we) {
					...
					int userPrompt = JOptionPane.showOptionDialog(null, "Are you sure you wish to exit?",
							Configuration.NAME, JOptionPane.DEFAULT_OPTION, JOptionPane.WARNING_MESSAGE, null, options,
							options[1]);
	...
	private void initializeMenuBar() {
		 ...
		 */
		/*
		 * menuPanel = new JPanel() { private static final long serialVersionUID = 1L;
		 * 
		 * @Override public void paintComponent(Graphics g) { int x = (this.getWidth() /
		 * 2 - (765 / 2)) + 2; int x2 = (this.getWidth() / 2 - 79) + (765 / 2);
		 * g.setColor(new Color(0x221F19)); scaledBG =
		 * BG.getScaledInstance(this.getWidth(), 55, Image.SCALE_SMOOTH);
		 * g.drawImage(scaledBG, 0, 0, null); g.drawImage(scaledLogo, this.getWidth() /
		 * 2 - 115, 2, null);
		 * 
		 *//* private final *//*
								 ...
								 *//* private final *//*
														 ...
														 * 
														 * for (int i = 0; i < 3; i++) { g.setFont(new Font("Verdana",
														 * Font.PLAIN, 12)); g.drawImage(leftHovers == i ? BUTTON_IMAGE
														 * : BUTTON_HOVER_IMAGE, x + (78 * i), 19, null);
														 * g.drawImage(rightHovers == i ? BUTTON_IMAGE :
														 * BUTTON_HOVER_IMAGE, x2 - (78 * i), 19, null);
														 * 
														 * drawCenteredString(g, leftLabels[i], (x + 32) + (70 * i) +
														 * TEXT_LEFT_POSITION_X[i], 34, TEXT_COLOR);
														 * drawCenteredString(g, rightLabels[i], (x2 + 45) - (70 * i) -
														 * TEXT_RIGHT_POSITION_X[i], 34, TEXT_COLOR); // if (leftHovers
														 * == i) { // menuPanel.setCursor(new
														 * Cursor(Cursor.HAND_CURSOR)); // drawTooltip(g,
														 * leftTooltips[i], x + (67 * i) + random(6), 15 - random(6));
														 * // } // if (rightHovers == i) { // int xOffset = 0; // if
														 * (rightHovers == 1) { // xOffset += 100; // } else if
														 * (rightHovers == 0) { // xOffset += 175; // } //
														 * menuPanel.setCursor(new Cursor(Cursor.HAND_CURSOR)); //
														 * drawTooltip(g, rightTooltips[i], x2 - xOffset - (67 * i) +
														 * random(6), 15 - random(6)); // } } } };
														 */
